...
/** Local alias kept for readability throughout this screen. */
type ContentType = JoinContentType;
...
export function JoinScreen() {
  ...
  const route = useRoute<RouteProp<MainTabParamList, 'Join'>>();
  ...
  // Deep links from the Discover feed preselect a content-type pill. Keyed on
  // the param so re-tapping the same section link re-applies the type.
  const paramContent = route.params?.content;
  useEffect(() => {
    if (paramContent) setContent(paramContent);
  }, [paramContent]);
